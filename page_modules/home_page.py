...
def show_home_page():
    ...
    # CO2 megtakarítás számítása az izzó konstans teljesítményéhez képest
    if st.session_state.get('calculate_co2_savings', False):
        st.write("---")
        ...
        if heater_power is None or heater_power <= 0:
            st.warning("⚠️ Kérjük, adjon meg egy érvényes izzó teljesítményt a navigációs sávban!")
        elif 'co2_hourly_dataframe' in st.session_state and st.session_state['co2_hourly_dataframe'] is not None:
            co2_hourly_df = st.session_state['co2_hourly_dataframe']
            
            # Az izzó konstans teljesítményével számolt CO2 kibocsátás
            ...
            # Napi összesítés az izzó CO2 kibocsátására
            heater_daily_co2 = co2_hourly_df_copy.groupby('Dátum').agg({
                'Izzó_CO2_g': 'sum'
            }).reset_index()
            heater_daily_co2.columns = ['Dátum', 'Izzó napi CO2 (g)']
            ...
            if 'co2_daily_dataframe' in st.session_state and st.session_state['co2_daily_dataframe'] is not None:
                ...
                # Megtakarítás számítása (Izzó CO2 - Adatbázis CO2)
                comparison_df['CO2 megtakarítás (g)'] = comparison_df['Izzó napi CO2 (g)'] - comparison_df['Napi CO2 (g)']
                comparison_df['Megtakarítás százalék'] = (comparison_df['CO2 megtakarítás (g)'] / comparison_df['Izzó napi CO2 (g)']) * 100
                
                # Összesített eredmények
                st.write("### Összesített eredmények")
                
                total_database_co2 = comparison_df['Napi CO2 (g)'].sum()
                total_heater_co2 = comparison_df['Izzó napi CO2 (g)'].sum()
                ...
                with col2:
                    st.metric("Izzó összes CO2", f"{total_heater_co2:.2f} g")
                with col3:
                ...
                display_comparison = comparison_df[['Dátum', 'Napi CO2 (g)', 'Izzó napi CO2 (g)', 'CO2 megtakarítás (g)', 'Megtakarítás százalék']].copy()
                display_comparison.columns = ['Dátum', 'Adatbázis CO2 (g)', 'Izzó CO2 (g)', 'Megtakarítás (g)', 'Megtakarítás (%)']
                ...
                fig_comparison.add_trace(go.Scatter(
                    x=comparison_df['Dátum'],
                    y=comparison_df['Izzó napi CO2 (g)'],
                    mode='lines+markers',
                    name='Izzó CO2',
                ...
                fig_comparison.update_layout(
                    title="CO2 Kibocsátás összehasonlítás: Adatbázis vs Izzó",
                    xaxis_title="Dátum",
                    yaxis_title="CO2 Kibocsátás (g)",
                    hovermode='x unified',
                    template="plotly_white",
        ...
        # Reset a flag-et, hogy ne jelenjen meg újra automatikusan
